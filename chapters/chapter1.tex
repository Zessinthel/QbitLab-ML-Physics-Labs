...
\textit{Esta práctica introduce el problema fundamental de la aproximación de funciones mediante modelos lineales en parámetros, con énfasis en la tensión entre capacidad de representación y generalización (sesgo-varianza). Se implementan tres familias de funciones base: monomios (matriz de Vandermonde), series de Fourier truncadas y funciones de base radial (RBF) gaussiana. Sobre tres funciones de prueba con distinta regularidad (una función periódica suave, la función de Runge y el valor absoluto), se construyen matrices de diseño para conjuntos fijos de entrenamiento y prueba. Los estudiantes resolverán el problema de mínimos cuadrados mediante descomposición en valores singulares (SVD), calculando el número de condición de cada matriz y las métricas de error RMSE, MAE y \(E_\infty\). Se analizará cuantitativamente el subajuste y el sobreajuste al variar el número de parámetros \(p\), y se relacionará el crecimiento del número de condición con la pérdida de precisión numérica, especialmente en la base monomial. Finalmente, se estudia el efecto del ruido gaussiano aditivo sobre el punto óptimo \(p^*\) y la norma de los parámetros, motivando la necesidad de la regularización. Los resultados permiten concluir que la elección de la base es crucial para la estabilidad y la generalización, y que la base de Fourier destaca por su excelente condicionamiento en problemas periódicos.}
...
Comprender la relación entre la elección de una familia de funciones base, el número de parámetros y la capacidad de generalización en modelos lineales, identificando los regímenes de subajuste y sobreajuste mediante el análisis del error y del condicionamiento numérico.

\subsection*{Objetivos específicos}
\begin{enumerate}[leftmargin=2em, label=\arabic*.]
  \item Implementar la construcción de matrices de diseño para bases de monomios, Fourier truncada y RBF gaussiana, y resolver el problema de mínimos cuadrados mediante SVD, calculando el número de condición \(\kappa(\mathbf{A})\) y las métricas de error RMSE, MAE y \(E_\infty\).
  \item Analizar cuantitativamente la evolución de los errores de entrenamiento y prueba al variar el número de parámetros \(p\), identificando el punto óptimo \(p^*\) y relacionando el sobreajuste con el crecimiento de \(\kappa(\mathbf{A})\) y de la norma de los parámetros.
...
\paragraph{Relación con la interpolación de Lagrange.}
Existe una base equivalente, la de Lagrange $\{L_j(x)\}$, que satisface
$L_j(\eqk{x_i})=\delta_{ij}$. En esa base, la matriz de diseño es la identidad y
los coeficientes son directamente los valores $\eqk{y_i}$. Ambas representaciones son
matemáticamente equivalentes, pero numéricamente la base monomial es muy inestable
(gran número de condición), mientras que la base de Lagrange evita resolver un sistema,
aunque sigue sufriendo el fenómeno de Runge en puntos equiespaciados.
...
con el grado del polinomio cuando los puntos $\eqk{x_i}$ no están en $[-1,1]$ o
...
\approx 10^{-16}$; véase \Cref{def:epsilon_mach}).
\end{observacion}
...
Chebyshev $\eqk{x_k} = \cos(k\pi/(p-1))$ en interpolación, o añadir viscosidad
...
\paragraph{Relación con la DFT.}
Si los puntos son equiespaciados, $\eqk{x_i} = a + (i-1)\frac{b-a}{n}$, la base anterior
es la versión real de la \textbf{Transformada Discreta de Fourier (DFT)}.
En notación compleja, $\phi_k(x)=e^{i k \eqcon{\omega_1} x}$ y la matriz de diseño (salvo un
factor de escala) es unitaria, lo que garantiza un número de condición óptimo.
La separación en senos y cosenos mantiene esa propiedad, como muestra el siguiente teorema.
...
Esta propiedad hace que la base de Fourier sea la preferida para señales en el dominio
del tiempo en esta práctica.
\end{teorema}
...
La matriz de Fourier con puntos equiespaciados es la base de la \textbf{Transformada Discreta de Fourier (DFT)}. En notación compleja, $\hat{\eqk{y}}_k = \sum_i \eqk{y_i} e^{-2\pi i k i/n}$; nuestra matriz real (senos y cosenos) es equivalente y hereda su ortogonalidad ($\kappa=\sqrt{2}$). 

En física experimental, la DFT se usa para:
\begin{itemize}
  \item Analizar espectros de frecuencia (oscilaciones láser, ruido en detectores).
  \item Detectar periodicidades en datos astronómicos (curvas de luz, púlsares).
  \item Reconstruir imágenes en MRI o CT (datos en espacio de Fourier).
\end{itemize}
La estabilidad de la base garantiza que el ruido no se amplifique en el espectro.
\end{fisicaconexion}
...
\begin{observacion}{RBF como núcleos de convolución gaussiana}
...
de la matriz RBF decrece al aumentar $\sigma$: bases muy estrechas generan
columnas casi ortogonales (buen condicionamiento), pero el modelo pierde
capacidad de interpolación global.
\end{observacion}

\begin{fisicaconexion}{RBF como funciones de Green}
En física, las RBF gaussianas aparecen como soluciones de la ecuación de calor:
$\eqfn{G}(x,t) = (4\pi t)^{-1/2} e^{-x^2/(4t)}$. Identificando $t = \sigma^2/2$, cada RBF
es el estado de un campo de difusión en tiempo $t$ a partir de una fuente puntual
en $c_j$. Barriendo $\sigma$ equivale a variar la escala de difusión: $\sigma$ pequeño
resuelve detalles finos (alta resolución, pero sensible al ruido); $\sigma$ grande
suaviza (baja resolución, robusto). Este compromiso es análogo al del filtro óptimo
de Wiener y motiva la regularización de Tikhonov (Práctica 2).
\end{fisicaconexion}
...
Dado el conjunto de entrenamiento $\mathcal{D}_{\mathrm{train}} = \{(\eqk{x_i}, \eqk{y_i})\}_{i=1}^n$ y un conjunto de prueba independiente $\mathcal{D}_{\mathrm{test}} = \{(\eqk{x_i'}, \eqk{y_i'})\}_{i=1}^{n_{\mathrm{test}}}$, se definen:

\begin{itemize}
  \item \textbf{Error de entrenamiento} (RMSE):
  \[
    E_{\mathrm{train}} \;=\;
    \frac{1}{\sqrt{n_{\mathrm{train}}}}\left\|
      \eqvec{A}_{\mathrm{train}} \equ{\boldsymbol{\theta}^*} - \eqk{\mathbf{y}}_{\mathrm{train}}
    \right\|_2,
  \]
  donde $\eqvec{A}_{\mathrm{train}}$ es la matriz de diseño evaluada en los
  puntos de entrenamiento $\{\eqk{x_i}\}$ y $\eqk{\mathbf{y}}_{\mathrm{train}}$ es el
  vector de observaciones correspondiente.

  \item \textbf{Error de generalización (prueba)} (RMSE):
  \[
    E_{\mathrm{test}} \;=\;
    \frac{1}{\sqrt{n_{\mathrm{test}}}}\left\|
  ...
  puntos de prueba $\{\eqk{x_i'}\}$ y $\eqk{\mathbf{y}}_{\mathrm{test}}$ son los
  valores reales (sin ruido, o con ruido independiente) en esos puntos.
\end{itemize}

Ambos errores miden la raíz del error cuadrático medio, pero el primero
evalúa el ajuste a los datos utilizados para entrenar, mientras que el
segundo mide la capacidad de predecir en puntos no vistos.
\end{definicion}
